...
from ...model.elasticache import Memcached
...
class MemcachedManager(ResourceManager):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.metadata_path = "metadata/elasticache/memcached.yaml"

    def create_cloud_service_type(self):
        result = []
        memcached_cst_result = make_cloud_service_type(
        ...
        result.append(memcached_cst_result)
        return result

    def create_cloud_service(
        self, region: str, options: dict, secret_data: dict, schema: str
    ):
        yield from self._collect_memcached_clusters(options, region)

    def _collect_memcached_clusters(self, options, region):
        region_name = region

        try:
            memcached_clusters, account_id = (
                self.connector.list_elasticache_memcached_clusters()
            )
            ...
            for cluster in memcached_clusters:
                try:
                    ...
                    link = f"https://{region}.console.aws.amazon.com/elasticache/home?region={region}#memcached:"
                    resource_id = cluster_id
                    reference = self.get_reference(resource_id, link)

                    memcached_vo = Memcached(cluster_data, strict=False)
                    cloud_service = make_cloud_service(
                        name=cluster_id,
                        cloud_service_type=self.cloud_service_type,
                        cloud_service_group=self.cloud_service_group,
                        provider=self.provider,
                        data=memcached_vo.to_primitive(),
                    ...
                    _LOGGER.error(
                        f'[list_elasticache_memcached_clusters] [{cluster.get("CacheClusterId")}] {e}'
                    )
            ...
            _LOGGER.error(f"[list_elasticache_memcached_clusters] [{region_name}] {e}")
            yield make_error_response(
                error=e,
                provider=self.provider,
                cloud_service_group=self.cloud_service_group,
