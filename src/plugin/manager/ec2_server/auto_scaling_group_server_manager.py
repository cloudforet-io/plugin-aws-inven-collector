...
class AutoScalingGroupManager(BaseManager):
    ...
    def __init__(self, ec2_connector=None):
        super().__init__()
        self.ec2_connector = ec2_connector

    def get_auto_scaling_info(
        self, instance_id, auto_scaling_groups, launch_configurations
    ):
        """
        ...
        match_autoscaling_group, match_launch_configuration = (
            self.get_auto_scaling_group_from_instance_id(
        ...
        if match_autoscaling_group is not None:
            ...
            if match_launch_configuration is not None:
                auto_scaling_group_data.update(
                    {
                        "launch_configuration": {
                            "name": match_launch_configuration.get(
                                "LaunchConfigurationName", ""
                            ),
                            "arn": match_launch_configuration.get(
    ...
    @staticmethod
    def get_auto_scaling_group_from_instance_id(
        instance_id, auto_scaling_groups, launch_configurations
    ):
