...
class LoadBalancerManager(BaseManager):
    ...
    def __init__(self, ec2_connector, **kwargs):
        super().__init__(**kwargs)
        self.ec2_connector = ec2_connector

    def get_load_balancer_info(
        self, load_balancers, target_groups, instance_id=None, instance_ip=None
    ):
        """
        ...
        """
        load_balancer_data_list = []
        match_load_balancers = self.get_load_balancers_from_instance_id(
            instance_id, instance_ip, load_balancers, target_groups
        )
        ...
        for match_load_balancer in match_load_balancers:
            ...
            load_balancer_data_list.append(load_balancer_data)

        return load_balancer_data_list

    def get_load_balancers_from_instance_id(
        self, instance_id, instance_ip, load_balancers, target_groups
    ):
        matched_lb_arns = []
        match_load_balancers = []
        match_target_groups = self.match_target_groups(
            target_groups, instance_id, instance_ip
        )

        for match_tg in match_target_groups:
            for lb in self.match_load_balancers(
                load_balancers, match_tg.get("LoadBalancerArns", [])
            ):
