...
class DomainValidationOptions(Model):
    ...
    validation_status = StringType(
        deserialize_from="ValidationStatus",
        choices=("PENDING_VALIDATION", "SUCCESS", "FAILED"),
    )
    resource_record = ModelType(ResourceRecord, deserialize_from="ResourceRecord")
    validation_method = StringType(
        deserialize_from="ValidationMethod", choices=("EMAIL", "DNS")
    )


class RenewalSummary(Model):
    renewal_status = StringType(
        deserialize_from="RenewalStatus",
        choices=("PENDING_AUTO_RENEWAL", "PENDING_VALIDATION", "SUCCESS", "FAILED"),
    )
    domain_validation_options = ListType(
        ModelType(DomainValidationOptions), deserialize_from="DomainValidationOptions"
    )
    renewal_status_reason = StringType(
        deserialize_from="RenewalStatusReason",
        choices=(
...
class Certificate(Model):
    ...
    revocation_reason = StringType(
        deserialize_from="RevocationReason",
        choices=(
            'UNSPECIFIED',
            'KEY_COMPROMISE',
            'CA_COMPROMISE',
    ...
    key_algorithm = StringType(
        deserialize_from="KeyAlgorithm",
        choices=(
            'RSA_2048',
            'RSA_1024',
            'RSA_4096',
